...
# Data params
thermo_cases = c("h2o_only_no_cont","h2o_only_simple_rh0.3",
				 "h2o_300_rh0.75_gamma5")
Tslist       = seq(270,300,by=10)
Ts_cases     = paste("h2o_only_no_cont_",Tslist,"K",sep="")
...
for (n in 1:Nthermo){
    ...
    points(coo1d_fac*coo1d,p_fac*p_s,type="l",lwd=lwd,col=col)  
}
legend("bottomright",legend=legendvec_thermo,col=colvec_thermo,lwd=2,
		cex=cex_leg)

# Theory var_thermo 
plot_H_frame("(b)  Theory, variable RH, lapse")
for (n in 1:Nthermo){
	case = thermo_cases[n]
	p_s  = eval(as.name(paste("p_s",case,sep="_")))
	col  = colvec_thermo[n]
	cts_verysimple = eval(as.name(paste("cts_verysimple1d",case,sep="_")))
    points(coo1d_fac*cts_verysimple,p_fac*p_s,type="l",lwd=lwd,col=col)  
}
...
legend("topleft",legend=Tslist,lwd=2,cex=cex_leg,col=colvec_Ts,
        title=expression(T[s]~~"(K)"))

# Theory varsst
plot_H_frame("(d)  Theory, variable Ts")
for (n in 1:NTs){
    Ts    = Tslist[n] 
    case  = Ts_cases[n]
    col   = colvec_Ts[n]
    ...
    p_s   = eval(as.name(paste("p_s",case,sep="_")))
    cts_verysimple1d = eval(as.name(paste("cts_verysimple1d",case,sep="_")))
    points(coo1d_fac*cts_verysimple1d,p_fac*p_s,type="l",lwd=lwd,col=col)  
}
legend("bottomleft",legend=Tslist,lwd=2,cex=cex_leg,col=colvec_Ts,
        title=expression(T[s]~~"(K)"))
