...
for (gas in gases){
	print(paste("plotting ",gas," case",sep=""))
	case      = eval(as.name(paste("case",gas,sep="_")))
	
	# Plot params
	# y coord
	fields 	   = c("kappa","coo","lntau")
	k_lim_h2o  = 1e-2*c(k_rot,k_vr) # cm^-1  
	...
	# PDF
	file = paste("~/Dropbox/17rad_cooling2/plots_paper/",gas,"_rfm_theory.pdf",
				sep="")
	...
	kappa   = ncvar_get(nc,sigmavar,start=c(1,mref),count=c(nk,1))*N_avo/m_mol # m^2/kg
	...
	# lntau
	main		 = bquote("(b)"~~ln~D*tau[tilde(nu)]*", RFM")
	...
	main		  = bquote("(c)"~~H[tilde(nu)]*", RFM  ("*K/day/cm^{-1}*")")
	...
	main = bquote("(e)"~~ln~D*tau[tilde(nu)]*", SSM2D")
	...
	# CTS  (s lev)
	cts2d_theory = calc_cts2d(k_fit,p,tabs_s,tau_theory_i)
	main		 = bquote("(f)"~~H[tilde(nu)]*", SSM2D  ("*K/day/cm^{-1}*")")
