...
% in a continuous way, because it collapses all cooling onto the $\tauk=1$ contour and does not allow emissivity-to-space to be distributed in the vertical.
...
\begin{table}[h]
	\begin{center}
		\begin{tabular}{c | | c | c} 
			...
			Wavenumber							  & $k$	      &  \cminverse  \\
			Mass absorption cofficient (with $T$, $p$ dependence)	  & $\kappa$	      &  $\meter^2/\kg$  \\
			Reference mass absorption cofficients (at $T$= 300 K, $p$ = 1000 hPa)	  & \kapparef,\ $\kappa_{\htwo}$,\ $\kappa_{\cotwo}$ 	      &  $\meter^2/\kg$  \\
 		    Optical depth at wavenumber $k$ & \tauk		   & dimensionless \\							       
 		    Transmissivity  at wavenumber $k$ & $\trans_k\equiv \exp(-\tauk)$		   & dimensionless \\							       
		    Optical depth exponent     & $\beta\equiv \der{\ln \tauk}{\ln p}$	& dimensionless \\							       
 		    Planck function at wavenumber $k$, temperature $T$ & $B(k,T)$		     & $\Wmsq/\mathrm{sr}/\cminverse$ \\							       
		   ...
		   Effective emitting width    & $\Delta k$ 	   & \cminverse \\	
		   Spectrally integrated transmissivity gradient   & $\overline{\ppp \trans_k}$ 	   & $\cminverse/\hPa$ \\	
			...
			Band-maximum reference absorption coefficients  & \begin{tabular}{@{}c@{}}$\kapparot = 260\ \meter^2/\kg$\\ $\kappavr = 10\  \meter^2/\kg$ \end{tabular} & $\kappaQ = 165\ \meter^2/\kg$ \\ \hline  
			Spectroscopic decay parameter 			  & \begin{tabular}{@{}c@{}}$\lrot = 60\ \cminverse$ \\ $\lvr = 42\ \cminverse $  \end{tabular} & $\lQ = 13\ \cminverse$   \\  \hline
		\end{tabular}
...
%		\caption{Test of the \Ts-invariance of flux divergences $\ppt F(T)$, for both RFM (left) and our simple 1D model (right) when expressed in temperature coordinates [Eqn. \eqnref{pptf_simple}]. \Ts-invariance holds for our 1D RFM calculations away from the surface, and this is captured by the simple model.
%		\label{pptf_tinv}
%		}
%	\end{center}
%\end{figure}
...
\begin{figure}[h]
	\begin{center}
		...
		\caption{As for Fig. \ref{h2o_rfm_theory}, except for the 500-850 \cminverse\ \cotwo\ band. The SSM2D emulates the behavior of RFM, and in particular reproduces the stratospheric enhancement of \chk.
...
\begin{figure}[h]
	\begin{center}
		...
		\caption{Profiles of the effective absorption coefficient $\kappaone$ for \cotwo, as predicted by \eqnref{eqn_kappaone} (red) as well as diagnosed from RFM (black) by linearly averaging $\kappa(k,p)$ over those $k$ which also contribute to $\Delta k$ in \eqnref{eqn_deltak}. The good agreement confirms the $1/p$ scaling for \kappaone, which underlies the stratospheric enhancement of \chk\ (Fig. \ref{co2_rfm_theory}).
		\label{kappaone}
		}
	\end{center}
\end{figure}
