...
tauvals_ssm  = kappavals_ssm%o%(ps/pref*(tabs/Ts)^(g/Rd/Gamma)*WVP)
kvec	     = 1:nk_rot
...
# PDF
file = "~/Dropbox/17rad_cooling2/plots/kink_breakdown.pdf"
...
main = bquote("(a)"~~~-partialdiff[p]*T[k]~",  RFM   ("*1/hPa*")")
...
plot(1,type="n", 
	xlim 	 = dtrans_dp1dlim, 
	ylim	 = plim,
	xlab 	 = expression(-bar(partialdiff[p]*T[k])~~~"("*c*m^{-1}/h*P*a*")"),
	ylab     = plab,
